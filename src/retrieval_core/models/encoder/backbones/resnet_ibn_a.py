...
model_urls = {
    'resnet50': '/media/csebastian/data/models/resnet50_ibn_a.pth',
...
def resnet50_ibn_a(pretrained=False, **kwargs):
    ...
    if pretrained:
        model.load_state_dict(torch.load(model_urls['resnet50'])['state_dict'])
    return model
...
def resnet101_ibn_a(pretrained=False, **kwargs):
    ...
    if pretrained:
        model.load_state_dict(torch.load(model_urls['resnet101'])['state_dict'])
    return model
...
def resnet152_ibn_a(pretrained=False, **kwargs):
    ...
    if pretrained:
        model.load_state_dict(torch.load(model_urls['resnet152'])['state_dict'])
    return model


def convert_model(model_path, new_model_path):
    # Removes module in state_dict keys
